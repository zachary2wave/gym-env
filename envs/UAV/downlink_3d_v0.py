...
class Downlink_3d_v0(gym.Env):
    def __init__(self):
        ...
        self.SPplacez = np.random.randint(0, 30, self.NSP)  # 节点位置y
        self.G = np.random.uniform(10, 30, self.NSP)  # 每个节点的数据量 M为单位
        self.P = 10  # 初始发射功率dBm
        ...
        if self.SNR[self.cline] <= self.SNRlimit:
            self.cline = -1
        # 定义状态空间
        a = spaces.Box(low=-self.amax, high=self.amax, shape=(1,), dtype=np.float32)
        p = spaces.Box(low=0, high=self.Pmax, shape=(1,), dtype=np.float32)
        self.action_space = np.array([a, a, a, p])
        ...
        # 状态格式 前面是[相对位置,剩余数据量]*NUAV 后面是数据量
        tempspace = [p_space, p_space, p_space, Gleft] * self.NSP
        tempspace.append(o_space), tempspace.append(o_space), tempspace.append(o_space)
        tempspace.append(v_space), tempspace.append(v_space), tempspace.append(v_space)
    ...
    def reset(self):
        ...
        # self.SPplacex = np.random.randint(-200, 200, self.NSP)  # 节点位置x
        # self.SPplacey = np.random.randint(-200, 200, self.NSP)  # 节点位置y
        self.G = np.random.uniform(10, 30, self.NSP)  # 每个节点的数据量
        self.time = 0
        ...
        if self.SNR[self.cline] <= self.SNRlimit:
            self.cline = -1
        S = []
        repositionx, repositiony, repositionz  = self.relative_position()
        temp = np.concatenate(([repositionx], [repositiony], [repositionz], [self.G]), axis=0)
        S = np.reshape(np.transpose(temp), (4 * self.NSP, 1))
        S = np.append(S, [self.placex, self.placey, self.placez])
        S = np.append(S, self.v)
        return S
    ...
    def step(self, a):
        ...
        if self.SNR[self.cline] <= self.SNRlimit:
            ...
            if Gidea < self.G[self.cline]:
                ...
                reward = output / (P + PS + self.P_data) * 100
            else:
                output = self.G[self.cline]
                self.G[self.cline] = 0
                reward = output / self.delta / (P + PS + self.P_data) * 100
                # 删除
        ...
        # 限制约束范围
        if self.placex > 200:
            reward = -10e-2 * np.maximum(abs(self.placex) - 200, 0) - 0.1
            if self.boundary == 1:
                self.placex = 200
            if self.show == 1:
        ...
        if self.placey > 200:
            reward = -10e-2 * np.maximum(abs(self.placey) - 200, 0) - 0.1
            if self.boundary == 1:
                self.placey = 200
            if self.show == 1:
        ...
        # 飞机最小速度约束
        # if np.sum(np.abs(self.v)) < 5:
        #     done = 1
        #     reward = -100

        repositionx, repositiony, repositionz = self.relative_position()
        temp = np.concatenate(([repositionx], [repositiony], [repositionz], [self.G]), axis=0)
        S_ = np.reshape(np.transpose(temp), (4 * self.NSP, 1))
        S_ = np.append(S_, [self.placex, self.placey, self.placez])
        S_ = np.append(S_, self.v)
        self.time += 1
    ...
    def relative_position(self):
        ...
        replacez = self.SPplacez - self.placez
        return replacex, replacey, replacez
