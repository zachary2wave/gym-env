...
class Downlink_2d(gym.Env):
    ...
    def step(self, a):
        # without sigmoid
        acc = np.array([a[0]*self.amax,a[1]*self.amax])
        self.P = a[2] * self.Pmax
        # with sigmoid
        # acc = np.array([math.tanh(a[0])*self.amax ,math.tanh(a[1])*self.amax])
        # self.P = self.sigmoid(a[2])*self.Pmax
