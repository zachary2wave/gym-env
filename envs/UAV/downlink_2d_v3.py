...
class Downlink_2d_v3(gym.Env):
    ...
    def reset(self):
        ...
        temp = np.concatenate(([repositionx], [repositiony], [self.G]), axis=0)
        S = np.reshape(np.transpose(temp), (3 * self.NSP, 1))
        S = np.append(S, [self.placex, self.placey])
    ...
    def step(self, a):
        ...
        S_ = np.append(S_, [self.placex, self.placey])
